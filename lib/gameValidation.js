...
GameValidation.prototype.start = function (callback) {
	...
	var err = null,
		kingSquareWhite = null,
		kingSquareBlack = null,
		resultWhite = {
			isCheck : false,
			isCheckmate : false,
	...
	if (this.game) {
		...
		kingSquareWhite = this.findKingSquare(piece.SideType.White);
		kingSquareBlack = this.findKingSquare(piece.SideType.Black);

		// find valid moves
		v.start(piece.SideType.White, setResult(this, resultWhite, v.isSquareAttacked(kingSquareWhite)));
		v.start(piece.SideType.Black, setResult(this, resultBlack, v.isSquareAttacked(kingSquareBlack)));
	} else {
		err = 'game is invalid';
	}

	if (callback) {
		callback(err, resultWhite, resultBlack);
	}
};
